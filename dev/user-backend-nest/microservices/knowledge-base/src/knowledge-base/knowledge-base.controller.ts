...
export class KnowledgeBaseController {
  ...
  async getKnowledgeBase(
    ...
    const cacheKey = `knowledge-base-${JSON.stringify(query)}`;
    const cachedKnowledgeBase =
      await this.cacheManager.get<KnowledgeBaseItem[]>(cacheKey);
    ...
    const knowledgeBase = await firstValueFrom(
      this.knowledgeBaseService.getKnowledgeBase(query),
    );
